    ...
    while fringe:
        ...
        if not node in explored:
            ...
            for successor in problem.expand(node):
                ...
                nextNode = (coordinate, nextActions)
...
def manhattanHeuristic(state, problem):
    ...
    distances = []
    for city_index in range(len(problem.cities)):
        currentCity = state[city_index]
        goalCity = goal_state[city_index]
        distance = abs(currentCity[0] - goalCity[0]) + abs(currentCity[1] - goalCity[1])
        distances.append(distance)

    return sum(distances)

def greedySearch(problem, heuristic=manhattanHeuristic):
    """Search the node with the lowest h (heuristic) first."""
    startState = problem.getStartState()
    frontier = [(heuristic(startState, problem), startState, [])]
    explored = set()
        ...
        if current_state not in explored:
            ...
            for nextState, action, cost in problem.expand(current_state):
                if nextState not in explored:
                    newPath = path + [action]
                    totalHeuristic = heuristic(nextState, problem)
                    heapq.heappush(
                        frontier, (totalHeuristic, nextState, newPath))
